...
const GEMINI_API_KEY = "AIzaSyBzwRvXE0rvIIBpUS2ddIHVGmiR9jujnIM";
...
// Function to analyze contextual questions based on selected text and full context
export async function analyzeWithContext({
  ...
  if (!selectedText.trim()) throw new Error("No selected text provided");
  if (!fullContext.trim()) throw new Error("No context provided");

  const contextualPrompt = `You are an expert trading analyst. A user has selected a specific portion of a trading analysis and wants to ask a question about it.
...
- If the question is unclear, ask for clarification`;
  ...
  try {
    // Generate cache key for contextual queries
    const contextData = JSON.stringify({ question, selectedText: selectedText.substring(0, 200), fullContext: fullContext.substring(0, 500) });
    let cacheKey: string;
    
    if (typeof window === 'undefined') {
      cacheKey = `context:${Buffer.from(contextData).toString('base64')}`;
    } else {
      try {
        cacheKey = `context:${window.btoa(unescape(encodeURIComponent(contextData)))}`;
      } catch (e) {
        cacheKey = `context:${Array.from(contextData).reduce((acc, char) => acc + char.charCodeAt(0).toString(16), '')}`;
      }
    }
    ...
    const cachedResponse = responseCache.get(cacheKey);
    if (cachedResponse) {
      console.log("Returning cached contextual response");
      return cachedResponse;
    }

    // Check for duplicate in-flight requests
    if (activeRequests.has(cacheKey)) {
      console.log("Contextual request already in progress, returning existing promise");
      return activeRequests.get(cacheKey)!;
    }

    console.log("Starting new contextual Gemini API request");

    // Create a promise for this contextual request
    const requestPromise = (async () => {
      try {
        // Add to queue for rate limiting
        const response = await apiQueue.add(
          async () => {
            ...
            return await result.response.text() || "No response from Gemini.";
          },
          { throwOnTimeout: true }
        );
    ...
    throw new Error(`Failed to analyze context: ${error.message || 'Unknown error'}`);
  }
}
